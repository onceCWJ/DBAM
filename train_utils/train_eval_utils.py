...
def train_one_epoch(model, optimizer, data_loader, device, epoch,
    ...
    for i, [images, targets] in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast(enabled=scaler is not None):
            loss_dict = model(images, targets, epoch)
            # print('loss_dict: {}'.format(loss_dict))
            # loss_dict分为两部分：classification 和 bound_box regression
            classification_loss = loss_dict['classification']
            bounding_box_loss = loss_dict['bbox_regression']
            # loss_dict['classification'] = classification_loss * 10
