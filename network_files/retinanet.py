...
from .losses import sigmoid_focal_loss, Cyclical_FocalLoss, ASL_FocalLoss, TStage_FocalLoss
...
class RetinaNetClassificationHead(nn.Module):
    ...
    def __init__(self, in_channels, num_anchors, num_classes, prior_probability=0.01):
        ...
        # output channels是anchors的数量与种类数的乘积
        # initial weights
        ...
        self.tstage_loss = TStage_FocalLoss()
        self.cyclical_loss = Cyclical_FocalLoss()
        self.asl_loss = ASL_FocalLoss()
    ...
    def compute_loss(self,
        ...
        for targets_per_img, cls_logits_per_img, matched_idxs_per_img in zip(targets, cls_logits, matched_idxs):
            ...
            '''
            losses.append(self.tstage_loss(
                cls_logits_per_img[valid_idxs_per_img],
                gt_classes_target[valid_idxs_per_img],
                epoch,
                reduction='sum'
            ...
            '''
            losses.append(self.asl_loss(
                cls_logits_per_img[valid_idxs_per_img],
                gt_classes_target[valid_idxs_per_img]
            ) / max(1, num_foreground))  # 注意这里除以的是正样本的个数
            
            
            losses.append(self.cyclical_loss(
                cls_logits_per_img[valid_idxs_per_img],
                gt_classes_target[valid_idxs_per_img],
                epoch
            ) / max(1, num_foreground))  # 注意这里除以的是正样本的个数
...
class RetinaNetHead(nn.Module):
    ...
    def compute_loss(self,
        ...
        return {
            "classification": self.classification_head.compute_loss(epoch, targets, head_outputs, matched_idxs),
...
class RetinaNet(nn.Module):
    ...
    在训练过程中，不仅需要输入的张量，还需要目标（目标是一系列的字典）
    ...
    Args:
        ...
        在backbone中，需要指定输出通道数目，用于确定每个特征图含有的通道数目
    ...
    def compute_loss(self, epoch, targets, head_outputs, anchors):
        ...
        return self.head.compute_loss(epoch, targets, head_outputs, anchors, matched_idxs)
    ...
    def forward(self, images, targets=None, epoch=None):
        ...
        # print('image.tensors.shape: {}'.format(images.tensors.shape))
        # 这里的Image.tensors似乎有些不太对劲
        ...
        if self.training:
            assert targets is not None
            losses = self.compute_loss(epoch, targets, head_outputs, anchors)
        else:
            # recover level sizes
